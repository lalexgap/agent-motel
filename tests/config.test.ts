...
describe("loadConfig", () => {
  test("defaults when no file exists", () => {
    expect(loadConfig()).toEqual({ defaultProvider: "codex", conciergeProvider: "codex", notifyOnIdle: true, idleNotifyMinSeconds: 30, remoteControl: true, preferSubagents: false, apiPort: 8787, apiBind: "127.0.0.1", worktreeByDefault: true, skipPermissions: true, commsMaxPerWindow: 5, commsWindowSeconds: 60, outboxTtlHours: 48, outboxPollSeconds: 2, outboxPollMaxSeconds: 30, tunnelPort: 2222, showUsage: true, gcAgentDays: 7, gcTrashDays: 30 });
  });
  ...
  test("file values override defaults, missing keys keep defaults", () => {
    ...
    expect(loadConfig()).toEqual({ defaultProvider: "codex", conciergeProvider: "codex", notifyOnIdle: true, idleNotifyMinSeconds: 120, remoteControl: false, preferSubagents: false, apiPort: 8787, apiBind: "127.0.0.1", worktreeByDefault: true, skipPermissions: true, commsMaxPerWindow: 5, commsWindowSeconds: 60, outboxTtlHours: 48, outboxPollSeconds: 2, outboxPollMaxSeconds: 30, tunnelPort: 2222, showUsage: true, gcAgentDays: 7, gcTrashDays: 30 });
  });
  ...
  test("defaultProvider defaults to codex and honors an override", () => {
    expect(loadConfig().defaultProvider).toBe("codex");
    writeFileSync(configFile(), JSON.stringify({ defaultProvider: "claude" }));
    expect(loadConfig().defaultProvider).toBe("claude");
  });
});

describe("shouldNotifyIdle", () => {
  const config: Config = { defaultProvider: "codex", conciergeProvider: "codex", notifyOnIdle: true, idleNotifyMinSeconds: 30, remoteControl: true, preferSubagents: false, apiPort: 8787, apiBind: "127.0.0.1", worktreeByDefault: true, skipPermissions: true, commsMaxPerWindow: 5, commsWindowSeconds: 60, outboxTtlHours: 48, outboxPollSeconds: 2, outboxPollMaxSeconds: 30, tunnelPort: 2222, showUsage: true, gcAgentDays: 7, gcTrashDays: 30 };
