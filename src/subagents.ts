...
export function renderSubagentScreen(turns: Turn[]): string[] {
  ...
  for (const turn of turns) {
    if (turn.kind === "user") {
      if (!isHarnessNoise(turn.text)) lines.push(`❯ ${clipMessage(turn.text, 120)}`);
...
const SALIENT_ARGS = ["command", "file_path", "pattern", "query", "url", "skill", "description", "sql", "text", "prompt", "function", "path"];
const ARG_CHARS = 100;
...
export function describeToolCall(name: string, input: string): string {
  ...
  const salient = SALIENT_ARGS.map((key) => args![key]).find((v) => typeof v === "string" && v.trim());
  const extra = label === "Skill" && typeof args.args === "string" && args.args.trim() ? ` ${args.args}` : "";
  if (typeof salient === "string") return `${label}(${clipMessage(salient + extra, ARG_CHARS)})`;
  const first = Object.values(args).find((v) => typeof v === "string" && v.trim());
  if (typeof first === "string") return `${label}(${clipMessage(first, ARG_CHARS)})`;
